...
This is achieved by using \textbf{implicit arguments} with curly brackets (we have already used them before):
\begin{lstlisting}[language=lean]
def Transitive {α : Type} (R : α → α → Prop) : Prop :=
  ∀ x y z, R x y → R y z → R x z
...
instance : Add Nat where
  add := Nat.add -- addition for natural numbers

instance : Add Int where
  add := Int.add -- addition for integers
\end{lstlisting}
