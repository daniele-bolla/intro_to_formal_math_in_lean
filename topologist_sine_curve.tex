...
As seen before, continuity is defined in the \lstinline[language=lean]|Topology|
type class, in terms of open sets.
One can create their own epsilon-delta definition given a notion of distance.
Another common definition of continuity is via limits and convergence.
...
The concept of a lmit is quiet extended, there are many types of limits to consider.
For intance the lmiti of a function at a point,
...
\end{itemize}
\begin{example}
  Coming back to limits and convergence, we can express various notions using filters:
  \begin{itemize}
  ...
  \end{itemize}
\end{example}
In Mathlib, \lstinline[language=lean]|Continuous| is a structure defined in terms of open sets,
as stated before, for global continuity. However, Mathlib also provides alternative definitions
for local continuity: \lstinline[language=lean]|ContinuousAt| defines continuity at a single point,
\lstinline[language=lean]|ContinuousWithinAt| defines continuity within a set at a point, and
\lstinline[language=lean]|ContinuousOn| defines continuity on an entire set.
All these local characterizations are defined in terms of filters, providing a more flexible
framework than the global open set definition.

The connection between the global continuity definition and the filter-based local definitions
is established by the following fundamental theorem:
...
This theorem shows that global continuity implies continuity at every point in the filter-based sense.
Using this bridge, we can derive the following corollary:
...
Thus, \lstinline[language=lean]|Continuous| implies \lstinline[language=lean]|ContinuousAt|,
establishing that global continuity is equivalent to pointwise continuity at every point.
The remaining local continuity concepts are defined similarly, all in terms of the limit
characterization:
...
Note that \lstinline[language=lean]|𝓝[s] x| denotes the neighborhood filter of $x$ restricted
