...
I have formalized a foundational counterexample in topology:
...
The topologist's sine curve elegantly bridges basic
point-set concepts with algebraic topology.
While Mathlib's broader topology library is quite mature,
specific counterexamples are often added later to
stress-test stabilized APIs, such as those for paths and connectedness.

Translating the informal proof into Lean revealed interesting contrasts.
Showing that $(0,0) \in \overline{S}$ using filters closely mirrored the informal argument.
In standard mathematics, we seamlessly apply real arithmetic to the
path domain $[0,1] \subset \mathbb{R}$.
In Mathlib, however, this domain is the \lstinline[language=lean]|unitInterval| subtype.
Because subtypes do not automatically inherit their base type's arithmetic operations,
one is forced to constantly cast variables back and forth
between \lstinline[language=lean]|unitInterval| and \lstinline[language=lean]|ℝ|.
This friction is not an inherent limitation of formal proof,
but rather a contingency of Mathlib's
current design choices.

\section{Introduction to the topologist's sine curve}

\subsection{Connectivity}
Following standard literature such as Munkres' \textit{Topology} \cite{munkres}, a topological space $X$ is defined as \textbf{connected} if it cannot be split into two disjoint non-empty open sets. A more restrictive property is that of being \textbf{path-connected}, where any two points can be joined by a continuous path $\gamma : [0,1] \to X$.
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.45\linewidth}
    ...
    \end{tikzpicture}
  \end{minipage}
  \caption{Visual representation of connectedness vs. path-connectedness.}
  \label{fig:connectivity-diagrams}
\end{figure}

It is a well-known result that path-connectedness implies connectedness. However, the converse does not hold. The topologist's sine curve is the classical counterexample demonstrating this asymmetry.

\subsection{Setup and Formalization}
My original proof follows Conrad's paper (\cite{Conrad_connnotpathconn}),
with a few modifications
and some differences from the final and generalized
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot [CurveBlue, mark=*, mark size=2pt] coordinates {(0,0)};
...
We can inline these two lemmas into the main proof to get a final single proof term:
\begin{lstlisting}[language=lean]
lemma S_is_conn : IsConnected S :=
  isConnected_Ioi.image sine_curve <| continuous_id.continuousOn.prodMk <|
    ...
    ContinuousOn.inv₀ continuous_id.continuousOn (fun _ hx => ne_of_gt hx)
\end{lstlisting}
Reading from left to right (thanks to the \textbf{pipe} operator),
we are building up the proof by successive applications:
\begin{itemize}
...
\end{itemize}
The entire chain can be read as building the continuity proof
from the innermost function (the inverse) outward to the
complete sine curve function, which is then used to prove that $S$ is connected.
...
Separately, Lean 4 has powerful built-in library search capabilities.
By appending a question mark to certain tactics, Lean will automatically
search the imported library for applicable lemmas. For instance, t
he tactic exact? (or apply?) searches for theorems that can directly
close (or make progress on) the current goal
...
\end{lstlisting}
This proof is presented in a relatively compact form.
For our purposes, this level of reduction strikes a balance between brevity and readability.
...
and stays in $S$.
A subtle but highly convenient Mathlib design decision aids this simplification.
In traditional mathematics, simplifying $1/(1/x)$ to $x$ requires proving $x \neq 0$.
In Lean, however, division by zero is defined such that $1/0 = 0$.
While initially alarming to mathematicians, this convention
makes the map $x \mapsto x^{-1}$ a total function on $\mathbb{R}$.
Consequently, the theorem \lstinline[language=lean]|inv_inv|
(which states $(x^{-1})^{-1} = x$) holds unconditionally.
This allows the \lstinline[language=lean]|simp| tactic to cleanly resolve
the nested inverses in $\sin(1/(1/n\pi))$
without demanding a tedious side-proof of non-zero-ness.
...
to get a more self-contained proof:
\begin{lstlisting}[language=lean]
theorem T_is_conn : IsConnected T :=
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \draw [gray, line width=1.5pt, dashed] (0.153,0.25) -- (0.153,0);
      \draw [dotred, line width=2.5pt] (0,0) -- (0.153,0);

      \addplot [dotred, mark=*, mark size=2pt] coordinates {(0,0)};
      \node[below left, dotred, font=\small] at (0,0) {$p(t_0)$};

      \addplot [dotred, mark=*, mark size=2pt] coordinates {(0.153,0.25)};
      \node[above right, dotred, font=\small] at (0.153,0.25) {$p(t_1)$};

      \node[below, font=\small, dotred] at (0.153,-0.05) {$a$};
    \end{axis}
  \end{tikzpicture}
  \caption{Topologist's sine curve with the $\varepsilon$-ball around $p(t_0)$.
    Note that the ball is depicted as a square since
    Mathlib uses the $L^\infty$ norm on $\mathbb{R}^2$.}
...
$$
for $k \in \mathbb{N}$.
As $k \to \infty$, these $x$-values approach 0,
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \draw[dotred, dashed, line width=1.5pt] (-0.03, 1) -- (0.25, 1);
      \node[above left, dotred, font=\small] at (0, 1.0) {$1$};

      \addplot [CurveBlue!50, mark=*, mark size=2pt] coordinates {(0,0)};

      \addplot [dotred, mark=*, mark size=2pt, only marks] coordinates {
          (0.1273, 1) (0.0707, 1) (0.0489, 1)
          (0.0374, 1) (0.0303, 1) (0.0254, 1) (0.0219, 1) (0.0192, 1)
        };
    \end{axis}
  \end{tikzpicture}
  \caption{The sequence \texttt{xs\_pos\_peak} isolating the points where the curve reaches its maximum $y$-value of $1$ as $x$ approaches $0$.}
...
As path-connectedness is intimately tied to homotopies,
this space provides a natural testing ground for Mathlib's
fundamental group API, $\pi_1(X, x_0)$.
A logical next step is formalizing the Warsaw circle—a closed variant
connecting the sine curve's endpoints with an arc.
This space famously exhibits a trivial fundamental group but a
non-trivial first singular homology group. Ultimately, expanding
he library of formalized topological oddities ensures
Mathlib's algebraic topology machinery can robustly handle
the edge cases of mathematical reality.
