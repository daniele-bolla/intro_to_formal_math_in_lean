...
The existential rules can be interpreted as an 
infinite disjunction, so that existential 
elimination corresponds to a \emph{proof by cases}. 
In Lean, this is done via the \lstinline|match| 
construct, known as \emph{pattern matching}, 
...
\lstinline|match| construct. 
This mechanism is known as \emph{pattern matching}, 
a fundamental concept in functional programming. 

For example:
\begin{lstlisting}[language=lean]
example (h : ∃ n : Nat, n > 0) : Prop :=
  match h with
  | ⟨witness, proof⟩ => True  -- here we can use 'witness' and 'proof'
\end{lstlisting}

Conversely, the \emph{universal quantifier} may be regarded as a generalized function. Accordingly, in Lean, the proof of a universally quantified statement is given by constructing a function that produces a proof for each possible instance.
...
needs a proof ot forall elminiation


Good point for introducing polymorhpism in Lean.
I also can talk about type ionference since i 
haven't introduce Prop bnut used before.
It could be nice to give a proof of transitivity and what 
is a relation in type theory.
