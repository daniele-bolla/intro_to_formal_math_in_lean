...
\begin{enumerate}
  \item We define $f(n) = \left(\frac{1}{n\pi}, 0\right)$, which we will show converges to $(0,0)$ and stays in $S$.
  ...
  \item \textbf{Membership proof} (\lstinline[language=lean]|hf'|):
        ...
        \begin{itemize}
          ...
          \item For such $n$, we show $f(n) = \left(\frac{1}{n\pi}, 0\right)$ is in $S$ by noting that
                ...
                $$
        \end{itemize}
  \item Finally, \lstinline[language=lean]|mem_closure_of_tendsto| combines these facts:
        if a sequence eventually stays in $S$ and converges to $x$, then $x$ is in the closure of $S$.
\end{enumerate}
...
% The main and more consistent part is showng that T is not path-connected.
% Showing this informally alreadyu require constructing and poinmting oput vartious step in order to convice
% an ideal reader.

% One cna very infomrally show it and conjvince the reader that
% by conrtadddiction a path exisitng in the topologiust sine curve t
% will eventually oscillate at the point 0, breacking continuity a basic fact for path connecteddens.

% by getting more procesie we have toconstruct such a sequence
% adna again show that it eventually soscillate for demonsdtration.

% We can use the structure \lstinline[language=lean]|IsPathConnected|
% to set up the statement and see if we can argue similarly in Lean.
% \begin{lstlisting}[language=lean]
%   lemma S_is_path_conn : IsPathConnected S := by sorry
% \end{lstlisting}
% In the same file where \lstinline[language=lean]|IsPathConnected| is defined,
% \texttt{Topology/Connected/PathConnected.lean}, we find the theorem
% \lstinline[language=lean]|IsPathConnected.image|,
% stating that the image of a path-connected set under a continuous map is path-connected.
% Applying the theorem requires proving both the continuity and path-connectedness of the domain.
% \begin{lstlisting}[language=lean]
% lemma S_is_path_conn : IsPathConnected S := by
%   apply IsPathConnected.image
%   · sorry -- ⊢ IsPathConnected (Ioi 0)
%   · sorry -- ⊢ ContinuousOn sine_curve (Ioi 0) 
% \end{lstlisting}
% Let's start with the continuity proof,
% \lstinline[language=lean]|ContinuousOn sine_curve (Ioi 0)|.
% The function $x \mapsto (x, \sin(1/x))$ is continuous on  $(0, \infty)$ as the
% product of two functions continuous at the given domain: the identity map $x \mapsto x$
% and the map $x \mapsto \sin(1/x)$.
% Here the full proof in Lean:
% \begin{lstlisting}[language=lean]
% lemma inv_is_continuous_on_pos_real : ContinuousOn (fun x : ℝ => x⁻¹) (pos_real) := by
%   apply ContinuousOn.inv₀
%   · exact continuous_id.continuousOn
...
% \end{lstlisting}
% We use \lstinline[language=lean]|ContinuousOn f S|
% to express that a function $f$ is continuous on a set $S$, wich is also used in the
% statement we need to prove.
% We define auxiliary lemmas \lstinline[language=lean]|inv_is_continuous_on_pos_real| and
% \lstinline[language=lean]|sin_comp_inv_is_continuous_on_pos_real| to respectively prove that
% the inverse function and the composition of sine with the inverse function
% are continuous on the positive reals.
% The theorem \lstinline[language=lean]|ContinuousOn.inv₀| states that if a function
% is continuous and non-zero on a set, then its inverse is continuous on that set.
% We provide two arguments:
% \begin{enumerate}
%   \item \lstinline[language=lean]|continuous_id.continuousOn| proves that the identity
%         map is continuous on any set;
%   \item \lstinline[language=lean]|intro x hx; exact ne_of_gt hx| proves
%         that $x \neq 0$ for all $x$ in $(0, \infty)$.
% \end{enumerate}
% In \lstinline[language=lean]|sin_comp_inv_is_continuous_on_pos_real|,
% we use the theorem
% \lstinline[language=lean]|continuous_sin.comp_continuousOn|,
% which states first that the sin fucntion is \lstinline[language=lean]|continuous_sin|,
% then given that we can access the a set of other theorems of conionuous functions, such as
% \lstinline[language=lean]|comp_continuousOn|, which states that
% the composition of a continuous function with a function
% that is continuous on a set is continuous on that set.
% We provide the previous lemma as an argument.
% Finally, we prove that the sine curve is continuous on the positive reals
% using \lstinline[language=lean]|ContinuousOn.prodMk|,
% which states that the product of two functions continuous on a set
% is continuous on that set. We provide the identity map and the sine funftion composed with inverse
% map as arguments.
% Proving such things is very common and there are obviously way to achive this with less woerk.
% Here a one liner proof, from the final version in Mathlib:
% \begin{lstlisting}[language=lean]
...
% The type class system in Lean automatically provides the required
...
% Now to prove that $(0, \infty)$ is path-connected, we can use the fact that
% it is a convex subset of the real numbers.
...
% Lean’s combination of \textbf{structures} and \textbf{type classes} allows for a highly
...
%   \item Using the $\varepsilon$-$\delta$ definition of continuity via Mathlib's \lstinline[language=lean]|Metric.tendsto_𝓝|
